...
import com.handa.springlite.annotation.RequestBody;
import com.handa.springlite.annotation.RequestParam;
import com.handa.springlite.annotation.ResponseBody;
import com.handa.springlite.annotation.RestController;
import com.handa.springlite.context.ApplicationContext;
import com.handa.springlite.context.ConfigurableApplicationContext;
import com.handa.springlite.exception.ErrorResponseException;
import com.handa.springlite.exception.NestedRuntimeException;
import com.handa.springlite.exception.ServerErrorException;
import com.handa.springlite.exception.ServerWebInputException;
import com.handa.springlite.io.PropertyResolver;
import com.handa.springlite.utils.ClassUtils;
import com.handa.springlite.web.utils.JsonUtils;
import com.handa.springlite.web.utils.PathUtils;
...
public class DispatcherServlet extends HttpServlet {
    ...
    public DispatcherServlet(ApplicationContext applicationContext, PropertyResolver propertyResolver) {
        ...
        this.resourcePath = propertyResolver.getProperty("${springlite.web.static-path:/static/}");
        this.faviconPath = propertyResolver.getProperty("${springlite.web.favicon-path:/favicon.ico}");
    ...
    public void init() throws ServletException {
        ...
        // 扫描注解为@Controller@RestController的Bean，拿到一组Dispatcher对象，存入getDispatchers和postDispatchers中，供后续处理请求时使用。
        ...
        for (Dispatcher dispatcher : dispatchers) {
            ...
            if (result.processed()) {
                ...
                if (dispatcher.isRest) {
                    // send rest response:
                    // 由于在处理的每一步都可以向HttpServletResponse写入响应，因此后续步骤写入时，应判断前面的步骤是否已经写入并发送了【HTTP Header】
                    if (!resp.isCommitted()) {
